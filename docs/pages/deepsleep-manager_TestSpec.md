...
|Description|HAL APIs|L2|L3|Control plane requirements|
|-----------|--------|--|--|--------------------------|
|Set the deep sleep with and check that the decrease in power comsumption based on the test configuration file with network standby disabled.|PLAT_DS_SetDeepSleep|N|Y|`NA`|
|Set the deep sleep with and check that the decrease in power comsumption based on the test configuration file with network standby enabled.|PLAT_DS_SetDeepSleep|N|Y|`NA`|
...
- Control plane must have way to capture the power consumption changes when the device enters deepsleep.